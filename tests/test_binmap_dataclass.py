...
class Temp(binmap.BinmapDataclass):
    temp: int = binmap.field("B")


@dataclass
class TempHum(binmap.BinmapDataclass):
    temp: int = binmap.field("B")
    humidity: int = binmap.field("B")
...
class AdvancedPad(binmap.BinmapDataclass):
    temp: int = binmap.field("B")
    _pad1: any = binmap.padding(2)
    humidity: int = binmap.field("B")
    _pad2: any = binmap.padding(3)
    _pad3: any = binmap.padding(1)
...
class TestPadClass:
    ...
    def test_parse_data(self):
        ...
        with pytest.raises(AttributeError) as excinfo:
            p._pad
        assert p.temp == 10
        assert "Padding (_pad) is not readable" in str(excinfo)
        assert p.humidity == 60
...
class ConstValues(binmap.BinmapDataclass):
    datatype: int = binmap.const("B", 0x15)
    status: int = binmap.field("B")
